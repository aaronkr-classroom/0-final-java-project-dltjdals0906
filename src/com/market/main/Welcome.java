			...
			while (!quit) {
				...
				if (flag) {
					...
					if (str.toUpperCase().equals("Y")) {
						...
						// 배열이동
						/* CartItem[] cartItem = new CartItemBook[NUM_BOOK];
						 * int num = 0;
						 * 
						 * for (int i = 0; i < mCartCount; i++)
	...
	public static void menuCartBill() throws CartException {
		...
		if(mCart.mCartCount==0)
			...
			if (str.toUpperCase().equals("Y")) {
				...
				System.out.print("배송받을 고객의 연락처을 입력하세요 ");
				String phone = input.nextLine();
				System.out.print("배송받을 고객의 배송지을 입력하세요 ");
				String address = input.nextLine();
	...
	public static void setFileToBookList(ArrayList<Book> booklist) {
		try {
			...
			while ((str2 = reader.readLine()) !=null) {
				...
				Book bookltem = new Book(readBook[0],readBook[1], Integer.parseInt
						(readBook[2]), readBook[3], readBook[4], readBook[5], readBook[6]);
				
				booklist.add(bookltem);
			}
			reader.close();
			fr.close();
		} catch (Exception e) {
